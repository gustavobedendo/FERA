...
class App():
    ...
    def __init__(self, addrel=None):
        ...
        try:
            ...
            self.directoriesFrame = tkinter.Frame(borderwidth=1, relief='groove')
            self.directoriesFrame.rowconfigure(0, weight=1)
            self.directoriesFrame.columnconfigure(0, weight=1)
            if(expertmode):
                self.dirs = ttk.Treeview(self.directoriesFrame, selectmode='browse', columns=('ID', '% Indexado', 'Tempo Decorrido (s)', 'Tempo Restante (s)', 'Paginas / Segundo', 'N Threads', 'Tipo'))
            ...
            if(expertmode):
                ...
                self.brem = tkinter.Button(self.continueFrame, text="Remover relatorio", command=self.remrels, image=self.imremove, compound="right")
            ...
            #self.abrir()
    def abrir(self):
        global exitFlag, nthreads, threads, threadp, root, continuar, ok
    ...
    def iterateXREF_NamedDests(self, doc, xref, p1, p2, p3, p4, pnotmm, xreftopage):
            ...
            if(len(destinations_limits)>1):
                ...
                if(len(quaislimites)>0 or 'null'==destinations_limits[0]):
                    ...
                    for i in range(grauavore):
                        ...
                        listax.extend(self.iterateXREF_NamedDests(doc, novoxref, p1, p2, p3, p4, pnotmm, xreftopage))
        return listax
    
    def iteratetreepages(self, xreftopage, doc, numberregex, xref, count):
        ...
        if(objrootpages=="/Pages"):
            ...
            for indobj, gen in numberregex.findall(objrootkids):
               count = self.iteratetreepages(xreftopage, doc, numberregex, indobj, count)  
            #return count
    ...
    def loadPages(self, xreftopage, doc, numberregex):
    ...
    def grabNamedDestinations(self, doc):
        #global xreftopage
        xreftopage = {}
        numbercompile = re.compile(r"([0-9]+)\s([0-9]+)")
        self.loadPages(xreftopage, doc, numbercompile)
        #
        ...
        try:
            ...
            if("xref" in key[0]):
                ...
                lista.extend(self.iterateXREF_NamedDests(doc, int(dests[1].split(" ")[0]),p1,p2, p3, p4, pnotmm, xreftopage))
                    ...
                    if(record[3]==hashpdf):
                        ...
                        if(varok.get()):
                            ...
                            try:
                                ...
                                try:
                                    nameddests = self.grabNamedDestinations(doc)
                                    #
                                except Exception as ex:
                                    nameddests = []
    ...
    def treeview_selection(self, event=None):
        try:
            ...
            #
            #
            if(len(self.dirs.get_children(selecao))==0):
                values = self.dirs.item(selecao, 'values')
                #
    ...
    def separateThread(self):
        global indexando, qtos, cont, totalPaginas, threads, continuar, qlchild
        ...
        try:
            ...
            for i in range(nthreads):
                ...
                threads[i] = mp.Process(target=insertThread, args=(processar, None, lock, lockp, processados, pathdb,continuar,))
                threads[i].start()
            indexando = 1
                ...
                for childroot in childrenroot:
                    ...
                    for child in childs:
                        ...
                        if(values[1]=='0%' and indexando == 0):
                            ...
                            #None
                            self.septhread = thr.Thread(target=self.separateThread)
                ...
                if(not processados.empty()):
                    ...
                    if(proc[0]=='update'):
                        ...
                        if(finalizados==nthreads):
                            hashpdf = str(md5(self.dirs.item(qlchild,'values')[7]))
                            #None
                            #None
                            sqliteconn = sqlite3.connect(str(pathdb))
                            cursor = sqliteconn.cursor()
    ...
    def popupcomandocancel(self, window):
        global marginsok
    ...
    def popupcomandook(self, window, datatop, databottom, dataleft, dataright):
    ...
    def checkmargin(self, fotocanvas, prop, datatop, databottom, dataleft, dataright, button_ok, pixorg):
        try:
            ...
            if(mmtopxbottom>=0 and mmtopxbottom> mmtopxtop and
                ...
                self.margemimg = self.create_rectanglex(x0k, y0k, x1k, y1k, (21, 71, 150, 85))
                margempreimage = fotocanvas.create_image(x0k, y0k, image=self.margemimg, anchor='nw', tags=("margem"))
    ...
    def setMargin(self, window, pdf):
        ...
        self.docimg = tkinter.PhotoImage(data = imgdata)
        
        docvanvas = fotocanvas.create_image((0,0), anchor='nw', image=self.docimg)
        ...
        button_ok = tkinter.Button(bottomframe, text="OK", command= lambda : self.popupcomandook(window, datatop, databottom, dataleft, dataright))
        button_ok.grid(row=0, column=0, sticky='ns', pady=5)
        button_cancel = tkinter.Button(bottomframe, text="Cancelar", command= lambda : self.popupcomandocancel(window))
        button_cancel.grid(row=0, column=1, sticky='ns', pady=5)
        
        dataleft.trace_add("write", lambda *args: self.checkmargin(fotocanvas, prop, datatop, databottom, dataleft, dataright, button_ok, pixorg))
        databottom.trace_add("write", lambda *args : self.checkmargin(fotocanvas, prop, datatop, databottom, dataleft, dataright, button_ok, pixorg))
        datatop.trace_add("write", lambda *args : self.checkmargin(fotocanvas, prop, datatop, databottom, dataleft, dataright, button_ok, pixorg))
        dataright.trace_add("write", lambda *args : self.checkmargin(fotocanvas, prop, datatop, databottom, dataleft, dataright, button_ok, pixorg))
        mmtopxtop = math.floor(datatop.get()/25.4*72)
    ...
    def addrelsPopup(self):    
        try:
            ...
            self.opcao.add_command(label='Laudo', command=partial(self.addrels,'laudo'))
            self.opcao.add_command(label='Relatorio', command=partial(self.addrels,'relatorio'))
            self.opcao.add_command(label='Outros', command=partial(self.addrels,'outros'))
    ...
    def addrelsfromviewer(self, tipo):
    ...
    def defineMargins(self, window, boolean):
        self.definirmargens = boolean
        window.destroy()
        
    def addrels(self, tipo, pathpdfinput = None):
        #None
        global pathdb, marginsok, mt, mb, me, md
        ...
        if(pathpdfs!=None):
            ...
            answeryes = tkinter.Button(window, text="SIM", command= partial(self.defineMargins,window, True))
                ...
                for pdf in pdfs:
                    ...
                    if(file_extension.lower()==".pdf"):
                        ...
                        if(r!=None):
                            ...
                            try:
                                if(not self.dirs.exists(str(relpdir))):
                                    self.dirs.insert('', index='end', iid=str(relpdir), text=str(relpdir), values=("","","","","","","",""))
                            except Exception as ex:
                                None
                                printlogexception(ex=ex)
                                None
...
def on_quit():
    global exitFlag, nthreads, threads, threadp, root, continuar
...
def createNewDbFile(addrel = None):
    global pathdb
    if(pathdb !=None):
...
def loaddb(event=None, lb1=None, addrel = None, pathdecided = False):
...
def go():
    global listapdfs, path, processar, somaq, lock, exitFlag, inserts, lockp, processados, root, importarDB, frameinitial, labelopcao, newdb, \
        olddb, continuar,qlchild,primeiraexec,pathdb, ok, expertmode, erros, warningimage, recenttxt, heightimage, widthimage, pathpdfinput
    exitFlag = False
    inserts = []
...
def showInfo(rootx, expertmodex, versionx, filein=None, f12Pressedx=None):
